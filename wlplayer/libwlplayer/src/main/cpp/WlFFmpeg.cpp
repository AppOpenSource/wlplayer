...
int WlFFmpeg::decodeFFmpeg() {
    ...
    if (audiochannels.size() > 0) {
        ...
        if (wlAudio->streamIndex >= 0 && wlAudio->streamIndex < pFormatCtx->nb_streams) {
            if (getAvCodecContext(pFormatCtx->streams[wlAudio->streamIndex]->codecpar, wlAudio) !=
                0) {
                exit = true;
                pthread_mutex_unlock(&init_mutex);
                return 1;
    ...
    if (videochannels.size() > 0) {
        ...
        if (wlVideo->streamIndex >= 0 && wlVideo->streamIndex < pFormatCtx->nb_streams) {
            if (getAvCodecContext(pFormatCtx->streams[wlVideo->streamIndex]->codecpar, wlVideo) !=
                0) {
                exit = true;
                pthread_mutex_unlock(&init_mutex);
                return 1;
    ...
    if (wlVideo != NULL) {
        ...
        if (mimeType != -1) {
            wlJavaCall->onInitMediacodec(WL_THREAD_CHILD, mimeType, wlVideo->avCodecContext->width,
                                         wlVideo->avCodecContext->height,
                                         wlVideo->avCodecContext->extradata_size,
                                         wlVideo->avCodecContext->extradata_size,
                                         wlVideo->avCodecContext->extradata,
                                         wlVideo->avCodecContext->extradata);
        }
...
//            LOGE("wlVideo 等待..........");
        ...
        if (ret == 0) {
            if (wlAudio != NULL && packet->stream_index == wlAudio->streamIndex) {
                ...
                if (mimType != NULL && !isavi) {
                    ...
                    av_bitstream_filter_filter(mimType,
                                               pFormatCtx->streams[wlVideo->streamIndex]->codec,
            ...
            av_free(packet);
            packet = NULL;
            if ((wlVideo != NULL && wlVideo->queue->getAvFrameSize() == 0) ||
                (wlAudio != NULL && wlAudio->queue->getAvPacketSize() == 0)) {
